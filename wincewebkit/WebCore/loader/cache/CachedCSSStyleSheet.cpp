...
void CachedCSSStyleSheet::data(PassRefPtr<SharedBuffer> data, bool allDataReceived)
{
    ...
    if (m_data) {
		...
		size_t blockCenterPos = m_decodedSheetText.find("display:block;text-align:center");
		if (static_cast<size_t>(-1) != blockCenterPos)
			m_decodedSheetText.insert("margin-left: auto;margin-right: auto;", blockCenterPos);
#endif
    }
    setLoading(false);
    checkNotify();
