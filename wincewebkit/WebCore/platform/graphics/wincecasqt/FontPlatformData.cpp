...
#if OS(WINCE) && ENABLE(DIVIDE_PAGES)
// added by achellies
#include "WebKit\wincecasqt\WebView.h"
#endif
...
#if OS(WINCE) && ENABLE(DIVIDE_PAGES)
    winFont.lfOutPrecision = 0;
    winFont.lfQuality = CLEARTYPE_QUALITY; //DEFAULT_QUALITY;
    winFont.lfClipPrecision = 0;
    winFont.lfPitchAndFamily = 0;
    winFont.lfItalic = italic;
	if (g_iReaderData)
		winFont.lfWeight = (0 == g_iReaderData->m_nFontType) ? 560 : FW_MEDIUM;
#else
