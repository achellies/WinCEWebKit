...
public:
        ...
        if (force || m_opacity < 1.)  {
            ...
            if (bmp) {
                ...
                case AlphaPaintImage:
                    ...
                    break;
                case AlphaPaintGradient:
                    bmp->setHasAlpha(true);
                    memset(bmp->bytes(), 0, bmp->bitmapInfo().numPixels() * 4);
                    return bmp;
    ...
    void paintBackTransparentLayerBitmap(HDC hdc, SharedBitmap* bmp, const IntRect& origRect, AlphaPaintType alphaPaint, const RECT& bmpRect)
    {
        ...
        if ((m_opacity < 1. || alphaPaint == AlphaPaintGradient || alphaPaint == AlphaPaintOther) && hasAlphaBlendSupport()) {
...
public:
    TransparentLayerDC(GraphicsContextPlatformPrivate* data, IntRect& origRect, const IntRect* rectBeforeTransform = 0, int alpha = 255, bool paintImage = false, bool paintGradient = false);
    ~TransparentLayerDC();
...
// SharedBitmap::getDC() is called, I.E., when m_bitmap is not null.
{
    ...
    if (paintGradient) {
        ...
        m_alphaPaintType = AlphaPaintGradient;
    } else if (!m_data->hasAlpha()) {
...
bool s_gradientsEnabled = true;
void GraphicsContext::setGradientsEnabled(bool flag)
{
    s_gradientsEnabled = flag;
}
...
Color gradientAverageColor(const Gradient* gradient)
{
    ...
    return Color((stop.red + lastStop.red) * 0.5f
        ...
        , std::min<float>(stop.alpha, lastStop.alpha));
}
...
void GraphicsContext::fillPath()
{
    ...
    OwnPtr<HBRUSH> brush;
    bool gradientHasAlpha = false;
    int averageColorAlpha = 255;

    if (m_common->state.fillGradient) {
        RefPtr<Gradient> fillGradient = m_common->state.fillGradient;
        if (s_gradientsEnabled) {
            gradientHasAlpha = fillGradient->hasAlpha();

            FloatRect rect(enclosingIntRect(m_data->mapRect(m_data->m_paths.at(0).boundingRect())));
            SetBrushOrgEx(m_data->m_dc, rect.x(), rect.y(), 0);
            brush = fillGradient->createBrush(rect);
        } else {
            Color averageColor = gradientAverageColor(fillGradient.get());
            averageColorAlpha = averageColor.alpha();
            if (!averageColorAlpha)
                return;
            brush = createBrush(averageColor);
        }
    ...
    if (m_data->m_opacity < 1.0f || m_data->hasAlpha() || gradientHasAlpha || averageColorAlpha != 255) {
        for (Vector<Path>::const_iterator i = m_data->m_paths.begin(); i != m_data->m_paths.end(); ++i) {
            ...
            trRect.inflate(1);
            TransparentLayerDC transparentDC(m_data, trRect, 0, averageColorAlpha, false, gradientHasAlpha);
            HDC dc = transparentDC.hdc();
            if (!dc)
                continue;
    ...
    if (numGlyphs > 1) {
        ...
        *curChar++ = *srcChar++;
        ...
        if (drawOneByOne) {
            ...
            ExtTextOut(m_data->m_dc, firstOffset, y, 0, NULL, text.data(), numGlyphs, NULL);
#else
        } else
